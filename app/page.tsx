...
export default function PopShoesHearingSheetMaker() {
  ...
  const exportPDF = () => {
    ...
    selectedGroups.forEach((group) => {
      const selectedPdfQuestions = group.pdfQuestions.filter((_, index) =>
        isQuestionSelected(group.id, index)
      );

      if (selectedPdfQuestions.length === 0) return;
      ...
      selectedPdfQuestions.forEach((q, index) => {
        if (y > 270) {
          doc.addPage();
          y = 20;
        }
    ...
    if (customQuestions.length > 0) {
      ...
      customQuestions.forEach((q, index) => {
        ...
        const lines = doc.splitTextToSize(`Custom ${index + 1}. ${q}`, 160);
        doc.setFontSize(10);
        doc.text(lines, 24, y);
        y += lines.length * 6 + 2;
        doc.line(24, y, 185, y);
                ...
                <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <p className="font-bold text-slate-500">
                      質問をクリックでON/OFFできます。PDFは英語で出力されます。
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button
